    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      if (t.controlSphere0) {
        ...
        if (dist < sumRadius0) {
          ...
          // Increased bounce effect: multiply by higher value for more bounce
          // More sensitive interaction with 3.5x force
          H.copy(j).multiplyScalar(Math.max(B.length(), 3.5));
          I.sub(j);
          B.sub(H);
          // Add upward bounce when cursor touches (flare effect)
          B.y += 0.15;
        }
      }
...
class Z extends d {
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0) {
        // Make cursor ball invisible by scaling to 0, but keep light active
        U.scale.setScalar(0);
